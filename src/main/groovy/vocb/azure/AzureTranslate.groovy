...
public class AzureTranslate {
	...
	Map trnJsonrunTrn(String text, String srcLang="en", String destLang="cs") {
		...
		JsonBuilder jsonBuilder = new JsonBuilder()
		String rq= jsonBuilder (
			[ "Text": text]
		)
		URL trnUrl = azEnv.translateUrl(srcLang, destLang)
		httpHelper.withUrlPostResponse(azEnv.httpHeaders, trnUrl, rq) {InputStream inp ->
	...
	static void main(String... args) {
		AzureTranslate bs = new AzureTranslate(httpHelper: new HttpHelper(), azEnv : new AzureEnv() )
		
		println bs.trnJsonrunTrn("clip")
